use std::{fs::OpenOptions, io::{BufRead, BufReader, BufWriter, Read, Seek, Write}, path::{Path, PathBuf}};

use crate::{base::GILLTTER_PATH, gilltter_add, objects::{self, tree::{Object, ObjectType}}};

pub fn add_one_in_index(filepath: &Path) -> anyhow::Result<()> {
    let index_file = OpenOptions::new().write(true).read(true).create(true).open(Path::new(GILLTTER_PATH).join("index"))?;
    ...
    let utils_sha1 = gilltter_add(filepath.to_str().unwrap());
